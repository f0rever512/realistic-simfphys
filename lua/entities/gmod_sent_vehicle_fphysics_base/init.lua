...
local function debounceStart(func, delay)
	local name = tostring(func) .. uuid()
	return function(...)
...
ENT.SendThrottleAnim = debounceStart(function(self)
...
local function UseCheck(ply, ent)
...
function ENT:SetPassenger( ply, instant )
	...
	ply.entering = true
	ply:DelayedAction('enterCar', 'Вход в машину', {
		time = instant and 0 or 1.75,
		check = function()
		...
		succ = function()
			...
			if driver then self.lockpicked = nil end
			ply.entering = nil
		end,
		fail = function()
			ply.entering = nil
