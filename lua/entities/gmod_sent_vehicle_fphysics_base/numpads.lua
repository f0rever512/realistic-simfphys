local function numpadFunction(key, pre, post)
	return function(sid, ent, keydown)
		if not IsValid(sid) or not IsValid(ent) or sid.inFlyEditor then return false end
		if isfunction(pre) then
			keydown = pre(sid, ent, keydown)
		end
		if ent.PressedKeys then
			ent.PressedKeys[key] = keydown
		end
		if isfunction(post) then post(sid, ent, keydown) end
	end
end
...
numpad.Register('k_hbrk_t', function(sid, ent, keydown)
	if not IsValid(sid) or not IsValid(ent) or not keydown then return false end
	ent:ToggleHandbrake()
end)
...
netstream.Hook('dbg-cars.belt', function(ply)
	...
	if not ply:GetNetVar('belted') then
		...
		-- RunConsoleCommand('say', '/me пристегнулся')
		ply:DelayedAction('belt', 'Застегивание ремня', {
			time = 1.5,
		...
		ply:DoAnimation(ACT_SIGNAL_HALT)
		-- RunConsoleCommand('say', '/me отстегнулся')
		ply:DelayedAction('belt', 'Расстегивание ремня', {
			time = 1.5,
			check = function() return IsValid(ply) and ply:Alive() and ply:GetVehicle() == seat end,
			succ = function()
...
numpad.Register('k_hrn', function(sid, ent, keydown)
	if not IsValid(sid) or not IsValid(ent) then return false end
...
numpad.Register('k_eng', function(sid, ent, keydown)
	...
	if keydown then
		if ent:EngineActive() then
			ent:StopEngine()
			netstream.StartPVS(sid:GetPos(), 'simfphys.anim', sid, 'engine', true)
			timer.Simple(0.3, function()
				if not IsValid(sid) then return end
				netstream.StartPVS(sid:GetPos(), 'simfphys.anim', sid, 'engine', false)
			end)
		else
			if sid.starting then return end
			sid.starting = true
			netstream.StartPVS(sid:GetPos(), 'simfphys.anim', sid, 'engine', true)
			sid:DelayedAction('engine', 'Запуск двигателя', {
				time = 1,
				check = function() return IsValid(ent) and IsValid(sid) and sid:Alive() and ent:GetDriver() == sid end,
				succ = function()
					sid.starting = nil
					ent:StartEngine(true)
					netstream.StartPVS(sid:GetPos(), 'simfphys.anim', sid, 'engine', false)
				end,
				fail = function() sid.starting = nil end,
			})
		end
	end
end)
...
numpad.Register('k_lock', function( sid, ent, keydown )
	...
	if keydown then
		if ent:GetIsLocked() then
			...
			if sid:KeyDown(IN_WALK) and istable(ent.pSeat) then
...
numpad.Register('k_turnmenu', function(sid, ent, keydown)
	...
	if not keydown then netstream.StartPVS(sid:GetPos(), 'simfphys.anim', sid, 'lhandle') end
end)

numpad.Register('k_flgts', function( sid, ent, keydown )
	if not IsValid(sid) or not IsValid(ent) or ent.preventEms then return false end
...
numpad.Register('k_siren', function( sid, ent, keydown )
	...
	if keydown then
		if sid:KeyDown(IN_DUCK) and not ent.siren then
		...
		if (Time - ent.KeyPressedTime) < 0.15 and not ent.siren and not sid:KeyDown(IN_DUCK) then
			ent.siren = true
			ent.cursound = 0
		end
...
numpad.Register('k_spcl', function( sid, ent, keydown )
	if not IsValid(sid) or not IsValid(ent) or not ent.HasSpecial then return false end
...
numpad.Register('k_lgts', function( sid, ent, keydown )
	if not IsValid(sid) or not IsValid(ent) or not ent.LightsTable then return false end
	if keydown then
		...
		netstream.StartPVS(sid:GetPos(), 'simfphys.anim', sid, 'rhandle')
