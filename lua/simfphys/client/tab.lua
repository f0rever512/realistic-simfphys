...
surface.CreateFont('real-simfphys.font', {
	font = 'Calibri',
	extended = true,
	size = 27,
	weight = 350,
...
local k_list = {
	{'Car / Автомобиль'},
	{KEY_W, 'cl_simfphys_keyforward', true, 'Forward / Газ'},
	{KEY_S, 'cl_simfphys_keyreverse', true, 'Reverse / Тормоз'},
	{KEY_A, 'cl_simfphys_keyleft', true, 'Left / Влево'},
	{KEY_D, 'cl_simfphys_keyright', true, 'Right / Вправо'},
	{KEY_I, 'cl_simfphys_keyengine', true, 'Engine / Зажигание'},
	{KEY_LALT, 'cl_simfphys_keyclutch', true, 'Clutch / Сцепление'},
	{KEY_LSHIFT, 'cl_simfphys_keywot', true, 'Speed up / Поддать газу'},
	{MOUSE_LEFT, 'cl_simfphys_keygearup', true, 'Gear up / Повысить передачу'},
	{MOUSE_RIGHT, 'cl_simfphys_keygeardown', true, 'Gear down / Понизить передачу'},
	{KEY_SPACE, 'cl_simfphys_keyhandbrake', true, 'Handbrake / Ручной тормоз'},
	{KEY_R, 'cl_simfphys_keyhandbrake_toggle', true, 'Handbrake toggle / Переключить ручной тормоз'},
	{MOUSE_MIDDLE, 'cl_simfphys_keymousesteer', true, 'Mouse steer / Поворот мышкой'},
	{KEY_F, 'cl_simfphys_lights', true, 'Lights / Свет'},
	{KEY_H, 'cl_simfphys_keyhorn', true, 'Horn / Сигнал'},
	{KEY_M, 'cl_simfphys_emssiren', true, 'Siren / Сирена'},
	{KEY_N, 'cl_simfphys_emslights', true, 'Special lights / Спецсигнал'},
	{KEY_COMMA, 'cl_simfphys_key_turnmenu', true, 'Turn menu / Поворотники'},
	{KEY_J, 'cl_simfphys_key_lock', true, 'Lock / Закрыть машину изнутри'},
	{KEY_LCONTROL, 'cl_simfphys_key_mirror', false, 'Mirror / Показать зеркало'},
	{KEY_B, 'cl_simfphys_key_belt', true, 'Belt / Застегнуть ремень'},
	{KEY_V, 'cl_simfphys_special', true, 'Truck winch / Лебедка тягача'},
}
...
local function buildclientsettingsmenu( self )
	...
	-- local panel = vgui.Create("DPanel", self.PropPanel)
	-- panel:SetPos( 20, 700 )
	-- panel:SetSize( 350, 25 )
	-- function panel:Paint(w, h)
	-- 	draw.RoundedBox(8, 0, 0, w, h, Color(0, 0, 0, 200))
	-- end
	-- createcheckbox(25, 705, 'Shadows from headlights', "cl_simfphys_shadows", self.PropPanel, 0) -- Enable Shadows
end
...
local function BuildClMenu( self )
	local panel = vgui.Create("DPanel", self.PropPanel)
	panel:SetPos( 0, 0 )
	panel:SetSize( 350, 25 )
	function panel:Paint(w, h)
	...
	createcheckbox(5, 5, 'Shadows from headlights / Тени от фар', "cl_simfphys_shadows", self.PropPanel, 0) -- Enable Shadows

	local panel = vgui.Create("DPanel", self.PropPanel)
	panel:SetPos( 0, 30 )
	panel:SetSize( 350, 670 )
	function panel:Paint(w, h)
		draw.RoundedBox(8, 0, 0, w, h, Color(0, 0, 0, 100))
	end
	...
	for _, v in ipairs(k_list) do
		if isstring(v[1]) then
			...
			l:SetFont('real-simfphys.font')
		else
			local b = buildBinder(panel, v)
			b:Dock(TOP)
			b:SetTall(25)
...
hook.Add( "SimfphysPopulateVehicles", "AddEntityContent", function( pnlContent, tree, node )
	...
	local node = tree:AddNode('Client Settings', "icon16/wrench_orange.png" )
	node.DoPopulate = function( self )
		if self.PropPanel then return end
