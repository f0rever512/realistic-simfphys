...
function colorMeta:Lerpclr(lerpValue, lerpTo)
...
local dashChecks = {
	function(car)
		return icons.left, (car.flashnum or 0) > 0.75 and car.signal_left and cols.orange or cols.bg
	end,
	...
	function(car)
		...
		local col = lamps and cols.lightFar
			...
			or cols.inactive
		-- if not car:GetEngineActive() and lights then col = col:Lerpclr(flash(3), cols.danger) end
		return lamps and icons.lightFar or icons.lightClose, col
	end,

	function(car) return
		icons.lock, car:GetIsLocked()
	end,

	function(car)
		local belt = LocalPlayer():GetNetVar('belted')
		local col = belt and cols.active or cols.inactive
		if (car.speed or 0) > 1 and not belt then col = col:Lerpclr(flash(3), cols.danger) end
	...
	function(car)
		...
		if not car:GetEngineActive() and not brake then col = col:Lerpclr(flash(3), cols.danger) end
		return icons.brake, col
	end,

	function(car)
		return car:GetEMSEnabled() and icons.alarm, cols.bg:Lerpclr(flash(4), cols.active)
	end,

	function(car)
		return (car:GetFuel() / car:GetMaxFuel()) < 0.2 and icons.fuel, cols.inactive:Lerpclr(flash(3), cols.danger)
	end,

	function(car)
		return icons.right, (car.flashnum or 0) > 0.75 and car.signal_right and cols.orange or cols.bg
	end,
}
...
local function drawDash(car)
	...
	local colRPM = lastRPM < 0.7 and cols.rpm or cols.rpm:Lerpclr((lastRPM - 0.7) / 0.3, cols.danger)
	...
	local colFuel = fuel > 0.7 and cols.orange or cols.orange:Lerpclr(1 - fuel / 0.7, cols.danger)
	draw.NoTexture()
	for i = 1, 10 do
		local height = i * 2 + 2
	...
	-- 	yalign = TEXT_ALIGN_CENTER,
	-- }
	-- draw.DrawText(car.speed, 'dbg-cars.hud.normal', 0, -26, cols.active, TEXT_ALIGN_CENTER)
	draw.SimpleTextOutlined(car.speed, 'dbg-cars.hud.normal', 0, -11, cols.active, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 150))
...
local function checkLook(ply, pos)
	return ply:GetAimVector():Dot((pos - ply:EyePos()):GetNormalized()) >= 0.95
end
