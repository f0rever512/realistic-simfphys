...
local function exitPlayer(ent, sender)
	if nextKeyDown - CurTime() >= 0 then return end
	nextKeyDown = CurTime() + 1
	
	local driver = ent:GetDriver()
	local HasPassenger = IsValid(driver)
	if driver:GetNetVar('belted') or driver.belting then
		local timeLeft = nextNotify - CurTime()
		if timeLeft < 0 then
			net.Start('lrp.cars-notify')
			net.WriteInt(1, 4)
			net.Send(driver)
			nextNotify = CurTime() + 5
		end
	end
	driver.exit = true
	driver:DelayedAction('exitCar', 'Выход из машины', {
		time = 1,
		check = function()
			if not IsValid(driver) then return false end
			if not IsValid(ent) then return false end
			if not driver:GetNetVar('belted') and not driver.belting then return true end
		end,
		succ = function()
		...
		inst = true,
		action = function()
			driver:DoAnimation(ACT_GMOD_GESTURE_ITEM_PLACE)
		end,
	})
end
...
CFG.use.gmod_sent_vehicle_fphysics_base = {
	...
	function(ply, ent)
		...
		return 'Сесть', 'octoteam/icons/door_open.png', function(ply, ent)
